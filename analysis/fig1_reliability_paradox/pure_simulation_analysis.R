...
#
# Unlike the empirical analysis, parameters are drawn fresh each iteration
# from controlled distributions — no empirical data required.
#
# USAGE:
...
create_experimental_designs <- function(n_trials = 20,
  ...
  # Generate experimental designs for Session A and Session B
...
recover_alpha_single_session <- function(responses, design, A, K, w0, w1, sigma_y) {
  # Estimate alpha from a single session using maximum likelihood
...
recover_alpha_joint <- function(responses_A, responses_B, design_A, design_B,
                                A, K, w0, w1, sigma_y) {
  # Estimate alpha jointly from both sessions
...
run_pure_simulation_bootstrap <- function(n_participants = 200,
                                          ...
                                          parallel = TRUE,
                                          show_progress = TRUE) {
  # Run bootstrap analysis with freshly generated participants each iteration
  ...
  for (manipulation in manipulation_types) {
    ...
    iteration_fn <- function(b, progress = NULL) {
      ...
      for (i in seq_len(n_participants)) {
        ...
        # Recover alpha from each session independently
        ...
        # Recover alpha jointly (both sessions, oracle w0/w1)
        recovered_alpha_joint[i] <- recover_alpha_joint(
          resp_A, resp_B, designs[['A']], designs[['B']],
          A_param, K_param, w0, w1, sigma_y
        )

        # Recover alpha with free response function (w0, w1 estimated)
...
create_figure_1 <- function(summary_data, trajectory_data = NULL) {
  ...
  # Panel b: Behavioral markers heatmap (3 rows x 6 columns)
  # Panel c: Cognitive markers heatmap (1 row x 6 columns)
  ...
  # Build spaghetti panel if trajectory data provided
  if (!is.null(trajectory_data)) {
...
cat('\n=== Starting bootstrap: N=200, 200 iterations, 6 conditions ===\n')

# Short protocol (20 trials)
cat('--- Short protocol (20 trials) ---\n')
...
cat('\n=== Bootstrap Complete ===\n')
cat('Results saved to: pure_simulation_results.RData\n\n')
...
# Check recovery ceiling
recovery_ceiling <- summary_data %>%
  filter(metric == 'alpha_recovery')
cat('\n=== Recovery Ceiling Check ===\n')
for (i in seq_len(nrow(recovery_ceiling))) {
  r <- recovery_ceiling[i, ]
  status <- if (r$mean_r > 0.85) 'PASS' else 'WARNING'
  cat(sprintf('  %s (n=%d, %s): r = %.3f [%s]\n',
              r$manipulation, r$n_trials, status,
              r$mean_r, sprintf('%.3f, %.3f', r$ci_lower, r$ci_upper)))
}

# Generate example trajectories for spaghetti panel
cat('\nGenerating example trajectories for panel a...\n')
...
cat('Outputs saved: fig1_reliability_paradox_pure.{png,pdf}, pure_simulation_{summary.csv,results.RData}\n')
