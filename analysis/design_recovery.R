# Design recovery: alpha identifiability across a trials x reinforcement grid,
# plus Fig 7 comparing model recovery (panel a) to summary tracking (panel b).
...
recover_params_joint_mle <- function(responses, design, A, K, sigma_y) {
  ...
  neg_log_likelihood_joint <- function(params) {
    alpha <- params[1]
    w0 <- params[2]
    w1 <- params[3]
...
run_identifiability_simulation <- function(
  ...
  # Draw fresh participants for each replicate (not shared across cells)
  ...
  simulate_one_design <- function(design_row, progress = NULL) {
    ...
    # Average over n_rep replicates with fresh participants each time
    r2_reps <- numeric(n_rep)
    n_valid_reps <- numeric(n_rep)
    for (rep in seq_len(n_rep)) {
      rep_participants <- generate_uniform_participants(n_participants)
      alpha_true <- rep_participants$alpha
      alpha_hat <- numeric(n_participants)
      ...
      for (i in seq_len(n_participants)) {
        ...
        responses <- simulate_conditioning_session(
          design,
          alpha = rep_participants$alpha[i],
          w0 = rep_participants$w0[i],
          w1 = rep_participants$w1[i],
          A = A_param,
          K = K_param,
        ...
        alpha_hat[i] <- recovery$alpha
      }

      valid <- complete.cases(alpha_true, alpha_hat)
      n_valid_reps[rep] <- sum(valid)
      if (sum(valid) >= 10) {
        r2_reps[rep] <- cor(alpha_true[valid], alpha_hat[valid])^2
      } else {
        r2_reps[rep] <- NA
      }
    }

    mean_r2 <- mean(r2_reps, na.rm = TRUE)
    ...
    data.frame(
      ...
      ratio = ratio,
      R2 = mean_r2,
      R2_ci_lower = quantile(r2_reps, 0.025, na.rm = TRUE),
      R2_ci_upper = quantile(r2_reps, 0.975, na.rm = TRUE),
      pearson_r = sqrt(mean_r2),
      spearman_rho = NA_real_,
      RMSE = NA_real_,
      relative_RMSE = NA_real_,
      MAE = NA_real_,
      bias = NA_real_,
      n_valid = round(mean(n_valid_reps)),
      convergence_rate = mean(n_valid_reps) / n_participants,
      stringsAsFactors = FALSE
    )
  }
  ...
  output_data <- file.path("../results", paste0(output_prefix, "_results.RData"))
  output_csv  <- file.path("../results", paste0(output_prefix, "_summary.csv"))
...
visualize_identifiability_heatmap <- function(results_summary,
  ...
  p_main <- ggplot(plot_data, aes(x = trials_factor, y = rate_factor, fill = R2)) +
    ...
    geom_text(
      ...
      color = ifelse(as.numeric(sprintf("%.2f", plot_data$R2)) < 0.5, "white", "gray20"),
      fontface = "bold",
      size = 5.5
    ) +
...
main <- function() {
  ...
  plot <- visualize_identifiability_heatmap(
    results,
    output_file = "../figures/alpha_identifiability_heatmap"
  )

  design_summary <- analyze_design_effects(
    results,
    output_file = "../results/alpha_identifiability_design_summary"
  )
...
# Slow panel-a factorial: skip if the cache exists.
cache <- file.path(dirname(getwd()), "results", "alpha_identifiability_results.RData")
if (!file.exists(cache)) {
  results <- main()
}


# Fig 7: model recovery (panel a) and summary tracking (panel b).
...
# Figure-local copies (compressed bodies; differ from the simulation helpers above).
simulate_conditioning_session_fig <- function(design, alpha, w0, w1, A, K, sigma_y) {
  n_trials <- nrow(design); v_plus <- 0; v_minus <- 0; responses <- numeric(n_trials)
  for (t in seq_len(n_trials)) {
    g <- design$cs_plus[t] * v_plus + design$cs_minus[t] * v_minus
    theta <- A + (K - A) / (1 + exp(-(w0 + w1 * g)))
    responses[t] <- rnorm(1, theta, sigma_y)
...
create_experimental_design_fig <- function(n_trials, reinforcement_rate, ordering, ratio) {
  if (ratio == "1:1") n_cs_plus <- n_trials / 2 else if (ratio == "1:2") n_cs_plus <- n_trials / 3 else n_cs_plus <- (2 * n_trials) / 3
  n_cs_plus <- round(n_cs_plus); n_cs_minus <- n_trials - n_cs_plus
  if (ordering == "blocked") {
    cs_sequence <- c(rep(1, n_cs_plus), rep(0, n_cs_minus))
  } else {
    cs_sequence <- sample(rep(c(1, 0), c(n_cs_plus, n_cs_minus)))
  ...
  cs_plus_trials <- which(cs_sequence == 1)
  shock_trials <- sample(cs_plus_trials, min(n_shocks, length(cs_plus_trials)))
  us_sequence <- numeric(n_trials); us_sequence[shock_trials] <- 1
  data.frame(cs_plus = cs_sequence, cs_minus = 1 - cs_sequence, us_plus = us_sequence)
}

generate_uniform_participants_fig <- function(n_participants) {
  data.frame(
    alpha = runif(n_participants, 0.05, 0.95),
    w0    = pmin(pmax(rnorm(n_participants, -1, 1.5), -5), 4),
    w1    = runif(n_participants, 2, 8)
  )
}

# slope, first-to-last change, and final value of the CS+ trajectory
csplus_summaries <- function(responses, design) {
  csp <- responses[design$cs_plus == 1]; nc <- length(csp)
  if (nc < 2) return(c(slope = NA, fl = NA, last = NA))
...
TRIALS <- c(10, 20, 30, 40, 60, 70, 80)
RATES  <- c(0.20, 0.30, 0.40, 0.55, 0.70, 0.85, 1.00)
A_param <- 0; K_param <- 100; sigma_y <- 5

cand <- c("alpha_identifiability_results.RData",
          file.path("..", "results", "alpha_identifiability_results.RData"))
cache <- cand[file.exists(cand)][1]
if (is.na(cache)) stop("alpha_identifiability_results.RData not found")
e <- new.env(); load(cache, envir = e)
panel_a <- e$results_summary[, c("trials", "rate", "R2")]

n_part <- 300; n_rep <- 10
panel_b <- do.call(rbind, lapply(seq_len(length(TRIALS) * length(RATES)), function(k) {
  tr <- TRIALS[((k - 1) %% length(TRIALS)) + 1]
  rt <- RATES[((k - 1) %/% length(TRIALS)) + 1]
  r2s <- numeric(n_rep)
  for (rp in seq_len(n_rep)) {
    pp <- generate_uniform_participants_fig(n_part)
    feats <- t(vapply(seq_len(n_part), function(i) {
      d <- create_experimental_design_fig(tr, rt, "random", "1:1")
      r <- simulate_conditioning_session_fig(d, pp$alpha[i], pp$w0[i], pp$w1[i], A_param, K_param, sigma_y)
      csplus_summaries(r, d)
    }, numeric(3)))
    df <- data.frame(alpha = pp$alpha, slope = feats[, 1], fl = feats[, 2], last = feats[, 3])
    df <- df[stats::complete.cases(df) & is.finite(rowSums(df)), ]
    r2s[rp] <- summary(lm(alpha ~ slope + fl + last, data = df))$r.squared
  }
  data.frame(trials = tr, rate = rt, R2 = mean(r2s))
}))

heat <- function(d, title) {
  d$trials <- factor(d$trials, levels = TRIALS)
  d$rate_lab <- factor(sprintf("%d%%", round(d$rate * 100)),
                       levels = sprintf("%d%%", round(RATES * 100)))
  ggplot(d, aes(x = trials, y = rate_lab, fill = R2)) +
    geom_tile(color = "white", linewidth = 1.2) +
    geom_text(aes(label = sprintf("%.2f", R2)),
              color = ifelse(as.numeric(sprintf("%.2f", d$R2)) < 0.5, "white", "gray20"), fontface = "bold", size = 4) +
    scale_fill_gradient2(low = "#D32F2F", mid = "#FF9800", high = "#4CAF50",
                         midpoint = 0.5, limits = c(0, 1), breaks = seq(0, 1, 0.25),
                         name = "Variance\nexplained") +
    labs(x = "Trials per session", y = "Reinforcement rate", title = title) +
    theme_minimal(base_size = 12) +
    theme(panel.grid = element_blank(),
          plot.title = element_text(face = "bold", size = 13),
          axis.title = element_text(face = "bold"),
          legend.position = "right", legend.key.height = unit(1.4, "cm")) +
    coord_fixed()
}

p_a <- heat(panel_a, "Model recovery") + labs(tag = "a")
p_b <- heat(panel_b, "Summary tracking") + labs(tag = "b")
fig <- p_a + p_b + plot_layout(guides = "collect") &
  theme(plot.tag = element_text(face = "bold", size = 14))

ggsave(file.path(fig_dir, "fig7_design.pdf"), fig, width = 11, height = 5, bg = "white", device = cairo_pdf)
ggsave(file.path(fig_dir, "fig7_design.png"), fig, width = 11, height = 5, bg = "white", dpi = 300)
