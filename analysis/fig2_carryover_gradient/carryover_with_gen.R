# =============================================================================
# Carry-over gradient WITH generalization present (gamma=0.15)
# Run the same carry-over gradient simulation but with cross-stimulus
# generalization active. Recovery models are SAME as before (naive and
# carry-over-correct), neither accounts for generalization.
# This shows that an additional unmodeled process further degrades reliability.
# =============================================================================
...
cat("=== Carry-Over Gradient + Generalization ===\n")
cat("Start:", format(Sys.time()), "\n\n")

out_dir <- "/Users/kenny/Library/CloudStorage/GoogleDrive-kennyyu0924@gmail.com/My Drive/Work/reliability/proof of concept/source 2"
...
GAMMA_TRUE     <- 0.15  # generalization present in data
...
recover_correct_carryover <- function(responses, design, A, K, w0, w1, sigma) {
  # Correct for carry-over (V0 estimation) but NOT generalization
  neg_ll <- function(par) {
...
plan(sequential)
summary_gen <- do.call(rbind, all_results)

# Print results
cat("\n=== RESULTS (carry-over + generalization gamma=0.15) ===\n\n")
for (df in DECAY_FACTORS) {
  sub <- summary_gen[summary_gen$decay_factor == df, ]
  cat(sprintf("  decay = %.1f:\n", df))
  for (m in c("naive_trt", "correct_trt", "behavioral_avg_trt")) {
    row <- sub[sub$metric == m, ]
    if (nrow(row) > 0)
      cat(sprintf("    %-22s  r = %6.3f  [%6.3f, %6.3f]\n",
                  m, row$mean_r, row$ci_lower, row$ci_upper))
  }
  cat("\n")
}

save(summary_gen, file = file.path(out_dir, "carryover_with_gen_results.RData"))
cat("Results saved.\nEnd:", format(Sys.time()), "\n")
