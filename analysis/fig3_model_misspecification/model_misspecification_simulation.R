# Model Misspecification Simulation - Source 3
#
...
if (requireNamespace("rstudioapi", quietly = TRUE) && rstudioapi::isAvailable()) {
  ...
  # If not in RStudio, check if we need to navigate to source 3
...
run_model_misspecification_simulation <- function(
  ...
  cat("Model misspecification simulation:", n_participants, "participants,",
      n_trials, "trials\n")
  ...
  cat("Simulating Pearce-Hall data and recovering with both models...\n")
  ...
  cat("Saved: model_misspecification_results.RData, model_misspecification_summary.csv\n")
...
create_model_misspecification_figure <- function(results_path = "model_misspecification_results.RData") {
  ...
  cat("Creating model misspecification figure...\n")
  ...
  # Combine panels - no title/subtitle (goes in figure caption)
  p_combined <- p_a + p_b +
    plot_layout(ncol = 2, widths = c(1, 1))

  # Save as PDF and PNG (Nature double column: 180mm = 7.09in)
  ggsave(
  ...
  cat("Saved: fig3_model_misspecification.pdf, fig3_model_misspecification.png\n")
